document.getElementById('daterange').addEventListener('click', function (event) {
    ...
    if (accountId && minDate && maxDate) {
        ...
        const confirmationMessage = `Filter Customer's Transactions From ${minDate.toLocaleDateString(undefined, options)} To ${maxDate.toLocaleDateString(undefined, options)}?`;
        ...
        if (rangeConfirmation) {
            const slugUrl = ''
            const rangeUrl = `/staff/transaction/history/filter/date/${encodeURIComponent(minDateStr)}/${encodeURIComponent(maxDateStr)}/${encodeURIComponent(accountId)}/`;
