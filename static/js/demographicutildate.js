document.getElementById('dateutilrange').addEventListener('click', function (event) {
    ...
    if (accountId && minDate && maxDate) {
        ...
        const confirmationMessage = `Filter Utility Demographics From ${minDate.toLocaleDateString(undefined, options)} To ${maxDate.toLocaleDateString(undefined, options)}?`;
        ...
        if (rangeConfirmation) {
            const slugUrl = ''
            const rangeUrl = `/staff/user/account/demographics/util/date/filter/${encodeURIComponent(minDateStr)}/${encodeURIComponent(maxDateStr)}/${encodeURIComponent(accountId)}/`;
